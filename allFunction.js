...
let tmpScore = 0.0000;
    ...
    let startTime = null;
    const MAX_DURATION = 14000; // 14s
    
    const WARMUP_MS = 2000; // karena awal video sering tidak sinkron (untuk kick)
    ...
    async function processFrames() {
        ...
        try {
            ...
            if (elapsed >= MAX_DURATION) {
                console.log("⏱ Penilaian selesai (14 detik)");
    ...
    function setupVideo(v1, v2) {
        ...
        v2.addEventListener('play', () => {
            if (v1.paused) {
                v2.pause(); // follower tidak boleh play sendiri
            }
        });
    ...
    // ------------------ END OF SCRIPT ------------------
    function syncFollower(master, follower) {
        if (!master || !follower) return;
        if (master.paused || master.ended) return;

        const drift = follower.currentTime - master.currentTime;

    // 1. HARD CORRECTION (drift besar)
    if (Math.abs(drift) > 0.25) { // 250 ms
        follower.currentTime = master.currentTime;
        follower.playbackRate = 1.0;
        return;
    }

    // 2. SOFT CORRECTION (drift kecil)
    if (drift < -0.05) {
        // follower tertinggal → percepat sedikit
        follower.playbackRate = 1.05;
    } else if (drift > 0.05) {
        // follower terlalu cepat → perlambat sedikit
        follower.playbackRate = 0.95;
    } else {
        // sinkron
        follower.playbackRate = 1.0;
    }
